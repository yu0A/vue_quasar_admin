...
import net_import from "src/pythonScripts/matplotlib/da_ti.py?url";
...
export default {
  ...
  mounted() {
    Promise.resolve()
      .then((res) => {
        setTimeout(() => {
          this.store.setFooterNote(
            "要使用NumPy库，首先要在Python脚本中编写 import matplotlib as mpl。"
          );
          // 加载Python环境
      ...
      .then((res) => {
        setTimeout(() => {
          this.pyodide.loadPackage("matplotlib");
        }, 3000);
      })
      .then((res) => {
        setTimeout(() => {
...
::v-deep
  div.q-field__control-container.col.relative-position.row.no-wrap.q-anchor--skip
  > div.q-field__control.relative-position.row.no-wrap {
  height: 70vh;
}
</style>
